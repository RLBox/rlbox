...
RSpec.describe "Authenticated Access", type: :request do
  ...
  describe "Navbar customization" do
    ...
    it "validates home page has proper padding for floating navbar" do
      ...
      first_section = doc.at_css('div > [data-testid="home-first-section"]')

      expect(first_section).to be_present,
        "Home page needs data-testid='home-first-section' on your first content section (as direct child of a container). " \
        "Add it to whichever element starts your page content. " \
        "Example: <div><section data-testid='home-first-section' class='pt-24'>...</section></div>"

      # Extract class attribute
      class_attr = first_section['class'] || ''

      # Check for top padding
      has_padding = class_attr.match?(/\bpt-(?:20|24|28|32)\b/)

      expect(has_padding).to be_truthy,
        "The first section with data-testid='home-first-section' needs top padding (pt-20, pt-24, pt-28, or pt-32) " \
        "to prevent content being hidden under the fixed navbar. Add the padding to the same element."
    end
  end
end
